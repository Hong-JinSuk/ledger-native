...
**표준 저장은 UTC, 사람이 읽는 곳만 KST(한국 시간).** (한국은 서머타임이 없어 **KST = 항상 UTC+9**, 고정 오프셋.)

- **표준 타임스탬프는 UTC로 저장한다.** `createdAt`/`updatedAt`은 `nowIso()`(= `new Date().toISOString()`, `...Z`)로 찍는다. 이 값들은 **Drive 동기화 병합**에서 "어느 편집이 최신인지" 비교하는 키라서 **하나의 절대 시간선(UTC)**이어야 한다.
  - ⚠️ **KST(로컬 시각)를 표준값으로 저장하지 말 것.** 오프셋 없는 로컬 문자열은 애매하고, 기기·타임존이 다르면 병합(충돌 해결)이 **깨진다.**
- **사람이 읽는 표면(개발 로그, DB 조회 등)에서만 KST로 변환**해 보여준다 — `src/lib/date`의 **`toKst(iso)` / `nowKst()`** 사용 (예: `2026-07-16 09:00:00+09:00`).
- **Supabase**: 컬럼은 `timestamptz`(내부 UTC) 그대로 둔다. DB에서 KST로 바로 보고 싶으면 **표준 컬럼을 바꾸지 말고** 읽기용 파생 컬럼/뷰를 판다:
  ```sql
  -- 표준 created_at(UTC)은 유지, 읽기 전용 KST 컬럼을 파생으로 추가
  created_at_kst text generated always as
    (to_char(created_at at time zone 'Asia/Seoul', 'YYYY-MM-DD HH24:MI:SS')) stored
  ```
  `auth.users`는 Supabase가 관리(UTC) — 건드리지 않는다. (현재 앱은 Supabase에 우리 테이블을 쓰지 않고 인증만 사용하므로 아직 팔 컬럼이 없다.)
- **가계부 데이터(Drive `ledger.json`)**: UTC 그대로. SQL로 브라우징하는 DB가 아니므로 **항목마다 KST 필드를 넣지 않는다**(용량·중복). 필요하면 읽을 때 `toKst()`로 변환.
...
- **Google Drive (사용자 소유)**: 실제 가계부 데이터의 원격 저장소. 데이터는 사용자 Drive의 **단일 파일 `ledger.json`**(전체 `LedgerSnapshot` 한 문서 = 거래·카테고리·예산·설정 전부)에 저장한다. Google 로그인으로 얻은 권한으로 사용자 본인의 Drive에 읽고 쓴다.
- **로컬(기기)**: 작성 중/오프라인의 1차 저장소. AsyncStorage 키 `ledger:snapshot:v1`에 같은 `LedgerSnapshot`을 보관한다(Drive `ledger.json`의 로컬 미러 = 항상 최신 작업본).

> 그래서 로그인은 **Google 로그인만** 지원한다. Drive 접근 권한이 필수이기 때문 (Credentials 로그인 등은 없음).

> ⚠️ **데이터 판단 기준 (작업 시 항상 적용)**: 거래·카테고리·예산 같은 "실제 가계부 데이터"의 원본은 **`ledger.json` 스냅샷(Drive) + 로컬 AsyncStorage 미러**다. `src/constants`의 `DEFAULT_CATEGORIES` 같은 값은 **첫 실행용 seed일 뿐** 기존 사용자의 실데이터가 아니다. 따라서 데이터 형태를 바꾸는 작업(예: 카테고리 정리)은 seed만 고치면 **신규 설치에만** 반영되고, 기존 데이터는 **스냅샷 마이그레이션(`src/lib/ledger/migrate.ts`) + local-first 병합 규칙**으로 바꿔야 한다.

> ⚠️ **현재 상태 (dev)**: 아직 정식 사용자는 없고, **개발자(레포 소유자) 본인의 Google 계정으로 로컬에서 테스트 중**이다. 즉 지금의 "사용자 데이터"는 개발자 본인의 테스트 데이터(로컬 AsyncStorage + 본인 Drive의 `ledger.json`)를 가리킨다. 데이터 보존/리셋 여부를 판단할 때 이 전제로 판단할 것 (실사용자 데이터 유실 위험이 아직 없다는 뜻이지, local-first 병합 규칙을 어겨도 된다는 뜻은 아니다).
...
### 스키마·데이터 마이그레이션 — ⚠️ CRITICAL (데이터 "형태"를 바꿀 때)

거래/카테고리/설정의 **형태**를 바꾸는 작업(카테고리 통합·필드 추가/이름 변경 등)은 `src/constants`의 `DEFAULT_*`(seed)만 고치면 **신규 설치에만** 반영된다. 기존 사용자의 실데이터(로컬 스냅샷 + Drive `ledger.json`)는 반드시 **버전 마이그레이션**으로 바꾼다. (이번 카테고리 통합이 그 사례.)

**레시피 (아래 넷을 한 세트로):**
1. `LEDGER_SNAPSHOT_VERSION`(`constants/ledger.ts`) +1.
2. `src/lib/ledger/migrate.ts`에 `migrateVN` 단계 추가 — **버전 게이트**(`if (version < N)`)로 한 번만 돌게. `migrateLedgerSnapshot`은 바뀐 게 없으면 **같은 ref 반환**(멱등).
3. `DEFAULT_CATEGORIES` 등 seed도 새 형태로 맞춘다(신규 설치용).
4. 순수 함수이므로 `migrate.test.ts`에 유닛 테스트.

**마이그레이션은 [위 병합 규칙]을 지켜야 한다 — sync-safe:**
- 은퇴/삭제 = **soft-delete tombstone**(`deleted:true` + `updatedAt` bump). 배열에서 물리 삭제 금지(합집합으로 부활함).
- 이름 변경/편집 = **살아있는 편집**(로컬 우선) + `updatedAt` bump.
- 거래는 카테고리를 **이름 문자열**로 참조 → 이름 remap 필수(id로 안 걸림).
- 이미 반영된 마이그레이션의 실수는 되돌리지 말고 **다음 버전 단계로 치유**(예: v3가 v2의 과잉 소분류를 제거).
- **카테고리 통합 패턴**(이번 사례): 자식 tombstone → 자식 이름을 부모 소분류로 편입(부모에 이미 있으면 생략) → 거래 이름 remap. (`migrateV2`의 쇼핑, `migrateV4`의 자동차→교통 참고.)

**자동 적용 지점:** `hydrate()`(로컬 로드 시) + `applySyncedSnapshot()`(Drive 스냅샷 병합 전). 화면/스토어 로직은 건드리지 않는다.

**⚠️ 마이그레이션은 "콜드 스타트"에서만 적용된다 (반드시 기억):**
- `hydrate()`는 앱 마운트 시 한 번만 돈다. **fast-refresh/핫 리로드는 hydrate를 다시 호출하지 않아**(메모리 스토어 유지), 코드가 번들에 들어가도 **완전 리로드 전엔 화면이 안 바뀐다.**
  - **앱(네이티브)**: 콜드 리로드(Metro `r` / 앱 재시작).
  - **웹**: 콜드 재시작(브라우저 껐다 켜기 / 하드 리프레시). fast-refresh만으론 안 됨.
- ⚠️ **웹과 앱은 로컬 저장소가 완전히 별개다** — 앱 = 기기 AsyncStorage, 웹 = **브라우저 localStorage**. 한쪽을 마이그레이션해도 다른 쪽은 자기 hydrate로 **각각 따로** 마이그레이션된다 → **플랫폼마다 각각 콜드 리로드** 필요.

**⚠️ `ledger.json`을 손으로만 편집하지 말 것:** local-first라 로컬 스냅샷이 병합에서 우선 → Drive만 고치면 다음 병합에서 rename 원복 / 삭제 부활. 데이터를 직접 바꿔야 하면 (a) 마이그레이션으로 하거나, (b) Drive 파일 교체 **+ 로컬 wipe**를 반드시 세트로.
...
- **부드러운 모션**: 화면 전환·목록 추가/삭제·펼침에 급작스러운 점프 대신 부드러운 애니메이션을 기본으로 (react-native-reanimated / Layout Animation). 감성의 상당 부분이 "움직임의 결"에서 온다.
  - **목록 등장은 stagger(계단식 페이드)를 기본으로 한다.** `.map()`으로 그리는 목록의 각 아이템을 `<FadeIn delay={i * 40}>`로 감싼다. ⚠️ `FadeIn`의 기본 `delay`는 0(단발)이라 stagger는 **호출부에서 index로 반드시 명시**해야 한다 — 빠뜨리면 전부 동시에 떠서 "결"이 사라진다. 간격은 40ms를 기본으로, 카드가 크고 개수가 적으면 ~70ms까지. **긴 목록은 총 지연이 늘어지지 않게 `Math.min(i, 8) * 40`처럼 상한**을 둔다. 부드러움은 `delay`가 아니라 `FadeIn`의 `duration`(≈350ms)·easing이 만드는 것이므로 그건 그대로 두고 `delay`(시작 시점)만 조정한다. 인위적 `setTimeout` 지연으로 "렌더 완료"를 흉내내지 말 것 — `FadeIn`이 `useEffect`(커밋 후)에서 시작하므로 타이밍은 이미 보장된다. 대량 `FlatList`는 스크롤 재활용으로 index-stagger가 어긋나니 첫 화면 분량에만 적용하거나 생략한다.
...
- **커밋 메시지**: 제목 한 줄로 끝낸다. 본문·불릿 설명을 덧붙이지 말고 한 줄로 요약할 것. 기존 관행대로 `feat:`/`fix:`/`docs:` 등 접두어 + 한국어. (실행 환경이 강제로 붙이는 `Co-Authored-By:` 트레일러는 예외 — 제목 한 줄 아래에 붙는다.)
